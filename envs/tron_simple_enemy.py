...
class TronEnvWithSimpleEnemy(TronBaseEnv):
    ...
    def step(self, action):
        ...
        old_pos = Position(self.agent_pos.x, self.agent_pos.y)
        ...
        enemy_killed = False
        if self.field.is_out_of_bounds(self.enemy.pos.x, self.enemy.pos.y) or self.field.state[self.enemy.pos.y, self.enemy.pos.x] == 3 or self.field.state[self.enemy.pos.y, self.enemy.pos.x] == 4 :
        ...
        if not done:
            if not enemy_killed:
                self.field.update_cell(old_enemy_pos.x, old_enemy_pos.y, 4)
